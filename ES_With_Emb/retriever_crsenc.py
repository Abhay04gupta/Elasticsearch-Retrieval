...
for size in Num_chunks:
    ...
    for idx, question in enumerate(questions, 1):
        ...
        if response.status_code == 200:
            ...
            print(f"Rerranking time: {rerank_time} seconds")
        else:
    ...
    # df.to_csv(f"analytics_top{size}_with_crsen_results.csv", index=False)
    # print(f"📁 Exported to analytics2_top{size}_with_crsen_results.csv")
    df.to_csv(f"qualitative_es_en.csv", index=False)
    print(f"📁 Exported to analytics2_top{size}_with_crsen_results.csv")