...
# Logging setup
logging.basicConfig(
    filename="result_rr.log",  # Log file name
    level=logging.DEBUG,
...
def build_range_around_date(center_date_str, months_before, months_after, field_name="date"):
    ...
    builds a Elastic Search filter expression for that range.

    Assumes Elastic Search stores dates in the format 'Month YYYY' (e.g., 'March 2024').
    """
...
async def search_topN_es(request: Request, data: Question):
    ...
    try:
        ...
        search_res = get_search_results(
            es, index_name, query_vector, ["text", "filename"],top_N
        )

        print("Number of chunks:",len(search_res[0]))
        ...
        # Retrieve the top N results directly without reranking
        top_n_results = [
            {
        ...
        logging.info(f"Total processing time (no rerank): {total_time:.4f} seconds")

        return {
            "question": data.question,
            "llm_query": llm_query,
            "query_date": query_date,
            "retrieved_results": top_n_results,
